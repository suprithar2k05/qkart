  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      ...
      if(response.data.success) {
        localStorage.setItem("balance", 5000 - getTotalCartValue(items))
        history.push('/thanks');
        enqueueSnackbar(
      ...
      enqueueSnackbar(
        "Unable to process order, try again!",
        {
          variant: "error",
        }
      );
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               {addresses.all.length < 1 ? <Typography my="1rem">
                 No addresses found for this account. Please add one to proceed
               </Typography> : addresses.all.map(({_id, address}) => <div onClick={() => selectAddress(_id)} className={`address-item ${addresses.selected.includes(_id) ? 'selected' : 'not-selected'}`} key={_id}>
                <span>{address}</span>
                <span>
                <Button
